...
def _is_label_match(goal: str, label: str) -> bool:
    g = _norm_label(goal)
    l = _norm_label(label)
    ...
    synonyms = {
        "plant": {"potted plant", "houseplant", "flower"},
        "tv": {"television", "monitor", "tv screen"},
        "sofa": {"couch"},
    }

    # 同义词双向
    if g in synonyms and l in synonyms[g]:
        return True
    if l in synonyms and g in synonyms[l]:
        return True
...
def pick_best_bbox(results, target_name: str, conf_th: float = 0.25):
    """
    results: self.obj_detector.predict(...) 的返回（ultralytics Results 列表）
    ...
    for r in results:
        ...
        for box in r.boxes:
            ...
            if conf < best_conf:
                continue
            if not _is_label_match(target_name, label):
                continue
...
def estimate_goal_position_from_yolo_bbox(
    bbox_xyxy, depth_image, agent_state,
    hfov_degrees: float, img_h: int, img_w: int,
    min_depth: float = 0.05
):
    """
    bbox_xyxy: (x1,y1,x2,y2) float/int
    depth_image: HxW, 单位米，和 color_sensor 对齐
    agent_state: habitat_sim.AgentState（里面有 sensor_states['color_sensor']）
    return: 世界坐标 np.array([x,y,z]) or None
    """
    ...
    # 取 bbox ROI 的稳健深度（median），过滤无效值
    roi = depth_image[y1:y2+1, x1:x2+1]
    valid = roi[np.isfinite(roi) & (roi > min_depth)]
    if valid.size == 0:
        # 兜底：中心 5x5
        r = 2
        y0, y1_ = max(0, v - r), min(img_h, v + r + 1)
        x0, x1_ = max(0, u - r), min(img_w, u + r + 1)
        roi2 = depth_image[y0:y1_, x0:x1_]
        valid2 = roi2[np.isfinite(roi2) & (roi2 > min_depth)]
        if valid2.size == 0:
            return None
        d = float(np.median(valid2))
    else:
        d = float(np.median(valid))

    # 像素反投影到相机坐标
    focal_length = calculate_focal_length(hfov_degrees, img_w)
    p_cam = np.array(unproject_2d(u, v, d, resolution=(img_h, img_w), focal_length=focal_length))

    # 相机坐标 -> 世界坐标（用 color_sensor 的位姿）
    sensor_state = agent_state.sensor_states["color_sensor"]
    p_world = local_to_global(sensor_state.position, sensor_state.rotation, p_cam)
    return p_world
