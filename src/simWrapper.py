...
class SimWrapper:
    ...
    def step(self, action: PolarAction):
        ...
        curr_state = agent.get_state() #取出当前state

        if action is PolarAction.null:
            new_agent_state = curr_state
        else: # 如果不是null动作 就新建一个
            new_agent_state = habitat_sim.AgentState()
            new_agent_state.position = np.copy(curr_state.position)
            new_agent_state.rotation = curr_state.rotation
            self._rotate_yaw(new_agent_state, new_agent_state.rotation, action.theta) # 旋转
            self._move_forward(new_agent_state, new_agent_state.position, new_agent_state.rotation, action.r) # 前进
            agent.set_state(new_agent_state)

        observation = self.sim.get_sensor_observations(0) # 获取observation
